...
bool GetMyExternalIP(CNetAddr& ipRet)
{
    ...
    for (int nHost = 1; nHost <= 2; nHost++)
    {
        ...
        if (nHost == 1)
        {
            ...
            addrConnect = CService("193.122.6.168",80); // checkip.dyndns.org   <--- modified 2023/05/04
...
void ThreadDNSAddressSeed(void* parg)
{
    ...
    RenameThread("chesscoin-dnsseed");
...
void ThreadDumpAddress(void* parg)
{
    ...
    RenameThread("chesscoin-adrdump");
...
void ThreadOpenConnections(void* parg)
{
    ...
    RenameThread("chesscoin-opencon");
...
void ThreadMessageHandler(void* parg)
{
    ...
    RenameThread("chesscoin-msghand");
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. ChessCoin 0.32% is probably already running."), addrBind.ToString().c_str());
        else
