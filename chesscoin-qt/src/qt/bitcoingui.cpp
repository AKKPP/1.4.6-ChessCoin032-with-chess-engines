...
#else
    resize(950, 660);
    setMinimumSize(950, 660);
#endif

    setWindowTitle(tr("ChessCoin 0.32%") + " - " + tr("Wallet"));
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a ChessCoin 0.32% address"));
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About ChessCoin 0.32%"), this);
    aboutAction->setToolTip(tr("Show information about ChessCoin 0.32%"));
    ...
    optionsAction->setToolTip(tr("Modify configuration options for ChessCoin 0.32%"));
    ...
    exportAction->setIconVisibleInMenu(true);
...
void BitcoinGUI::createToolBars()
{
...
#ifdef HELPCHESSMENU
    QToolBar *toolbar2 = addToolBar(tr("Export toolbar"));
    ...
    chessToolBar->setStyleSheet("color: lightgreen;");
    chessToolBar->addAction(chessPlayAction);

    const QRect& rt = geometry();

#ifdef Q_OS_WIN
    mainToolBar->setMinimumWidth(rt.width() - 125);
#else
    mainToolBar->setMinimumWidth(rt.width() - 150);
#endif
}
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("ChessCoin 0.32% client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("ChessCoin 0.32% client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to ChessCoin 0.32% network", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid ChessCoin 0.32% address or malformed URI parameters."));
    }
