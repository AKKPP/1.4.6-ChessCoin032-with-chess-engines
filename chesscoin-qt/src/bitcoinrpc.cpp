...
{
    return GetBoolArg("-testnet", false) ? 17324 : 7324;
}
...
string HTTPPost(const string& strMsg, const map<string,string>& mapRequestHeaders)
{
    ostringstream s;
    s << "POST / HTTP/1.1\r\n"
      << "User-Agent: chesscoin-json-rpc/" << FormatFullVersion() << "\r\n"
      << "Host: 127.0.0.1\r\n"
...
void ThreadRPCServer(void* parg)
{
    ...
    RenameThread("chesscoin-rpclist");
...
void ThreadRPCServer2(void* parg)
{
        ...
        (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"]))
    {
        ...
        string strWhatAmI = "To use chesscoind";
...
void ThreadRPCServer3(void* parg)
{
    ...
    RenameThread("chesscoin-rpchand");
...
Object CallRPC(const string& strMethod, const Array& params)
{
    ...
    fprintf(stderr, "Server[%s] Connected OK\n", GetArg("-rpcconnect", "127.0.0.1").c_str());
...
std::string HelpMessageCli(bool mainProgram)
{
    ...
    strUsage += "  -rpcport=<port>        " + _("Connect to JSON-RPC on <port> (default: 7324 or testnet: 17324)") + "\n";
