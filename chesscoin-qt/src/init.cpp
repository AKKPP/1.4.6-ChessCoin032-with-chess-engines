...
void Shutdown(void* parg)
{
    ...
    RenameThread("chesscoin-shutoff");
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("ChessCoin 0.32% version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  chesscoind [options]                     " + "\n" +
                  "  chesscoind [options] <command> [params]  " + _("Send command to -server or chesscoind") + "\n" +
                  "  chesscoind [options] help                " + _("List commands") + "\n" +
                  "  chesscoind [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "chesscoin:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("ChessCoin 0.32%"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("ChessCoin 0.32%"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -?                     " + _("This help message") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: chesscoin.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: chesscoind.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 7323 or testnet: 17323)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 7324 or testnet: 17324)") + "\n" +
...
//#endif
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. ChessCoin 0.32% is shutting down."));
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of ChessCoin 0.32%") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart ChessCoin 0.32% to complete") << "\n";
